...
case class Generator(grammarName: String, packageName: String, grammar: Grammar) {
  ...
  def generateIn(root: Path): Try[Unit] = {
    val worker = RuntimeGenerator(grammar)

    val preParserBytesWriter = new ByteArrayOutputStream()
    Using(new ObjectOutputStream(new GZIPOutputStream(preParserBytesWriter))) {_.writeObject(worker.parser)}
    ...
    val parser: Vector[String] = Vector(
      s"${INDENT}private val parser: eliseevh.parsergen.runtime.Parser = new ObjectInputStream(new GZIPInputStream(new ByteArrayInputStream(Base64.getDecoder.decode(\"$preParserBytes\")))).readObject().asInstanceOf[eliseevh.parsergen.runtime.Parser]$LS"
      )
    val lexer: Vector[String] = Vector(
      s"${INDENT}private val lexer: eliseevh.parsergen.runtime.Lexer = new ObjectInputStream(new GZIPInputStream(new ByteArrayInputStream(Base64.getDecoder.decode(\"$lexerBytes\")))).readObject().asInstanceOf[eliseevh.parsergen.runtime.Lexer]$LS"
      )
    val fromTree: Vector[String] =
      (grammar.terminals.collect { case desc@TerminalDescriptor(name) if desc.normal =>
        s"${INDENT * 2}case eliseevh.parsergen.runtime.Leaf(token) if token.descriptor.name == \"$name\" => `$name`(token.value.asInstanceOf[${desc.typeName}])$LS"
        ...
        case rule@NonTerminalRule(NonTerminalDescriptor(lhsName), rightSide, _) =>
          val collectedRightSide = rightSide.collect {
            case desc@TerminalDescriptor(name) if desc.normal =>
              s"eliseevh.parsergen.grammar.lexer.TerminalDescriptor(\"$name\")"
            case desc@NonTerminalDescriptor(name) if desc.normal =>
              s"eliseevh.parsergen.grammar.parser.NonTerminalDescriptor(\"$name\")"
          }
          s"${INDENT * 2}case eliseevh.parsergen.runtime.Node(rule, children, value) if rule.descriptor.name == \"$lhsName\" " +
        ...
        .prepended(
          s"${INDENT}private def fromTree(tree: eliseevh.parsergen.runtime.Tree[?]): $nodeClassName = tree match {$LS"
          )
