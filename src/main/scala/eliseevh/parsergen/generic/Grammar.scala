...
import eliseevh.parsergen.context.Generator
import eliseevh.parsergen.generic.grammar.NonTerminalRule
import eliseevh.parsergen.generic.lexer.{NormalTerminalRule, SkipTerminalRule, TerminalRule}
...
object Grammar {
  case class NamedGrammar(
      grammar: eliseevh.parsergen.grammar.Grammar,
      name: String
  ) {
  ...
  implicit class GrammarWrapper(grammar: eliseevh.parsergen.grammar.Grammar) {
  ...
  implicit def extractGrammar[R](
    ...
    if (named.view.distinct.size != named.size) {
      throw new IncorrectGrammarException("Terminals and non-terminals must all have different names")
    }
    eliseevh.parsergen.grammar.Grammar(
      start,
      grammar.nonTerminalRules.toSet,
      grammar.terminalRules.toSet,
